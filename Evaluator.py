...
class RankCollector():
    ...
    def prune(self, max_anom, min_anom):
        ...
        for i in range(len(self.all_anomalies)):
            ...
            if str(self.all_rels[i]) in self.unique_triples_materialized.keys() \
                    and str(self.all_rels[i]) not in rc.unique_triples_materialized.keys():
                rc.unique_triples_materialized[str(self.all_rels[i])] = self.unique_triples_materialized[str(self.all_rels[i])]
            if str(self.all_rels[i]) in self.total_unique_triples.keys() \
                    and str(self.all_rels[i]) not in rc.total_unique_triples.keys():
                rc.total_unique_triples[str(self.all_rels[i])] = self.total_unique_triples[str(self.all_rels[i])]
        return rc
